...
(function(window, document) {
    ...
    Taggle.prototype._fixInputWidth = function() {
        ...
        if (width == 0) {
            ...
            var taggle_items = tagglelist.getElementsByTagName('LI');
            var count_taggles = taggle_items.length;

            if (count_taggles > 0) {
                var i = 0;
                while (i < count_taggles){
                    width += taggle_items[i].offsetWidth;
                    ++i;
                };
            };
            
            var width_taggleList = tagglelist.offsetWidth;
            this._setInputWidth(width_taggleList - width - 50);
        }
        else {
            this._setInputWidth(width - leftPos - padding);
        };
