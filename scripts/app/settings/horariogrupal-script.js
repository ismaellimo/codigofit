...
function MostrarDatos () {
    $.ajax({
        ...
        success: function (data) {
            ...
            if (countdata > 0){
                while(i < countdata){
                    strhtml += '<li class="mdl-list__item dato pos-rel" data-id="' + data[i].tm_idhorariogrupal + '">';
                    
                    strhtml += '<input name="chkItem[]" type="checkbox" class="oculto" value="' + data[i].tm_idhorariogrupal + '" />';
    ...
    openModalCallBack('#modalRegistro', function () {
        if (idData != '0') {
            $.ajax({
                ...
                success: function (data) {
                    if (data.length > 0) {
                        $('#hdIdPrimary').val(data[0].tm_idhorariogrupal);
                        $('#txtRutina').val(data[0].tm_idrutinagrupal);
                        $('#txtInstructor').val(data[0].tm_idinstructorgrupal);
                        $('#txtAmbiente').val(data[0].tm_idambiente);
                        $('#txtDia').val(data[0].ta_diasemana);
