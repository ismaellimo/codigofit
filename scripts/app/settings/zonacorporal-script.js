$(function () {
    $('#gvDatos').on('click', '.mdl-button', function(event) {
        ...
        if (accion == 'edit'){
            ...
            MessageBox({
                content: '¿Desea eliminar la zona corporal?',
                width: '320px',
                height: '130px',
                buttons: [
                    {
...
function LimpiarForm () {
    $('#hdIdPrimary').val('0');
    $('#chkMaquina')[0].checked = false;
    $('#chkMedida')[0].checked = false;
    $('#txtNombre').val('').focus();
}
...
function MostrarDatos () {
    $.ajax({
        ...
        success: function (data) {
            ...
            if (countdata > 0){
                while(i < countdata){
                    strhtml += '<li class="mdl-list__item dato pos-rel" data-id="' + data[i].tm_idzonacorporal + '">';
                    
                    strhtml += '<input name="chkItem[]" type="checkbox" class="oculto" value="' + data[i].tm_idzonacorporal + '" />';
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.length > 0) {
                        ...
                        $('#chkMaquina')[0].checked = data[0].tm_maquina == '1' ? true : false;
                        $('#chkMedida')[0].checked = data[0].tm_medida == '1' ? true : false;
